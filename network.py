...
class ResNetFc(nn.Module):
    def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=True, class_num=65):
        super(ResNetFc, self).__init__()
        model_resnet = resnet_dict[resnet_name](pretrained=True)  # 采用预训练的模型
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        fc2_emb = self.bottleneck(x)
        logit = self.fc(fc2_emb)
...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.fc_params = nn.Sequential(nn.Linear(50 * 4 * 4, 500),nn.LayerNorm(500), nn.ReLU())
...
class AdversarialNetwork_v1(nn.Module):
    def __init__(self, in_feature, hidden_size):
        super(AdversarialNetwork_v1, self).__init__()
